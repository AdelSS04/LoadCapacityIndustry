	...
	public class OperatorsODBO
	{
		...
		public static bool UpOperatingNumber (OperatorsO op)
		{
			string requete = String.Format("update GrpOfOperators set NumberOfOperator={1}" +
				   " where ((OperationID='{0}' and YearT>={2}) and WeekT>={3}) ;", op.OperationID,op.NumberOfOperator,op.YearT,op.WeekT);
		...
		{
			string requete = String.Format("insert into GrpOfOperators (OperationID,YearT,WeekT,NumberOfOperator)  " +
